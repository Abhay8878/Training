  ...
  async createUserWithProfile(userData: CreateUserProfileDto) {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          'Preferences failed, but keeping User/Profile. Rolling back to savepoint.',
        );
        await queryRunner.query(
          'ROLLBACK TO SAVEPOINT core_registration_complete',
        );
      }
      ...
      return {
        message:
          'Registration complete (Transaction used Savepoints for Preferences).',
        user: savedUser,
      };
    } catch (err: any) {
      // GLOBAL ROLLBACK:
